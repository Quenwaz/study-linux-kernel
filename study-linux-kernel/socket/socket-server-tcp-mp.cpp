...
int process_connection(void * pData)
{
	int sock = *(int*)(pData);
	for(;;)
	{
		...
		fprintf(stderr, "[%d] send %d bytes msg: %s\n", sock, _n_read, szMsg);
	}
...
int main(int argc, char* argv[])
{
	if (argc != 2)
	{
		fprintf(stderr, "Usage: socket-server-tcp-mp [port] [backlog:options,default 4]\n");
		return EXIT_FAILURE;
	}
	...
	for (;;)
	{
        ...
        case -1:
            ...
            break;
        case 0:
            exit(process_connection(&sock_client));
        default:
            break;
        }
