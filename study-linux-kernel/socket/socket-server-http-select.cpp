...
std::string __make_http_response(uint32_t status, const std::string &content_type, const std::string &strContent)
{
    ...
    oss << "\r\n";
    oss << "Server: Select-I/O of HTTP/1.1";
    oss << "\r\n";
    oss << "Content-Type: ";
    oss << content_type;
    oss << "\r\n";
...
std::string make_http_response(const HttpRequest &httpReq, const std::string &remotehost)
{
    ...
    if (httpReq.header.method == Header::GET)
    {
        if (httpReq.header.resource == "/")
        {
            ...
            {         
                code = 404;
                snprintf(szContent, sizeof(szContent),
                    "<HTML><TITLE>欢迎使用Select-I/O of HTTP</TITLE><BODY><H1>Not Found</H1></BODY></HTML>");
            }else{
    ...
    }else if (httpReq.header.method == Header::POST){
        snprintf(szContent, sizeof(szContent),
            "<HTML><TITLE>欢迎使用Select-I/O of HTTP</TITLE><BODY><form><label for=\"querystring\">QueryString:</label><input type=\"text\" value=\"%s\" name=\"querystring\" id=\"querystring\"><br><label for=\"body\">Body:</label><input type=\"text\" value=\"%s\" name=\"body\" id=\"body\"></form></BODY></HTML>", httpReq.header.querystring.c_str(), httpReq.body.c_str());
    }else{
        code = 501;
        snprintf(szContent, sizeof(szContent),
            "<HTML><TITLE>欢迎使用Select-I/O of HTTP</TITLE><BODY><H1>%s</H1></BODY></HTML>", GetCodeDesc(code).c_str());
    }
...
int main(int argc, char *argv[])
{
    if (argc != 2)
    {
        fprintf(stderr, "Usage: socket-server-http-select [port] [backlog:options,default 4]\n");
        return EXIT_FAILURE;
    }
