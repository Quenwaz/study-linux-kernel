...
int main(int argc, char* argv[])
{
	...
	for (;;)
	{
		...
		int arrFds[2] = {STDIN_FILENO, sock_client};
		const int nfds = sizeof(arrFds)/sizeof(arrFds[0]);
		for (size_t i =0; i < nfds; ++i){
			FD_SET(arrFds[i], &setrd);
			FD_SET(arrFds[i], &setwd);
		}

		int ret = select(nfds, &setrd, &setwd, NULL, NULL);
		if (ret <= 0){
			continue;
		}

		for(size_t i =0;i < nfds; ++i){
			if (FD_ISSET(arrFds[i], &setrd)){

			}

			if (FD_ISSET(arrFds[i], &setwd)){
